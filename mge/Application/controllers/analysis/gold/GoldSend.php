...
class GoldSend extends CI_Controller{
	...
	public function index(){
		// 获取金币发放类型配置
		$query1 = $this->dbapp->manage_sp_gold_typpe_config_s(1);
		
		// 新增日期列和其他列
		array_unshift($query1, array('gname'=>'日期'));
		array_push($query1, array('gname'=>'总发放', 'tid'=>'total'));
		array_push($query1, array('gname'=>'其他', 'tid'=>'10000'));
		
		// 获取渲染字符串和对象
		$this->data['thList'] = $this->getThObj($query1);
		$this->data['colObj'] = $this->getColObj($query1);
		
		$this->parser->parse('gold/send.html', $this->data);
	}
